 ...
 *  
 */
package pl.orange.opl.storefront.security.cookie;
...
public class EnhancedCookieGenerator extends CookieGenerator
{
	 ...
	 * {@link #setEnhancedCookiePath(HttpServletRequest, Cookie)} method.
	 */
	...
	{
		return isHttpOnly() ? new HttpHeaderCookieDecorator(request, response) : new RequestPathCookieDecorator(request, response);
	}
	...
	class HttpHeaderCookieDecorator extends RequestPathCookieDecorator
	{
		...
		public Cookie decorate(final Cookie cookieIn)
		{
			...
			ServerCookie.appendCookieValue(headerBuffer, cookieIn.getVersion(), cookieIn.getName(), cookieIn.getValue(),
					cookieIn.getPath(), cookieIn.getDomain(), cookieIn.getComment(), cookieIn.getMaxAge(), cookieIn.getSecure(),
					isHttpOnly());
