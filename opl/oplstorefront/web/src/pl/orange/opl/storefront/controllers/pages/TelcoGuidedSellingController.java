...
import pl.orange.opl.facades.bundle.GuidedSellingFacade.ProductType;
import pl.orange.opl.facades.data.BundleTabData;
import pl.orange.opl.facades.product.TelcoProductFacade;
import pl.orange.opl.storefront.controllers.TelcoControllerConstants;
import pl.orange.opl.storefront.controllers.util.GlobalMessages;
import pl.orange.opl.storefront.forms.UpdateQuantityForm;
...
public class TelcoGuidedSellingController extends AbstractSearchPageController
{
	...
	{
		return editPositionalComponent(bundleNo, componentId, searchQuery, page, showMode, sortCode, request, model, 1, redirectAttributes);
	}
			...
			final int relativeposition, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		{
            redirectAttributes.addFlashAttribute(GlobalMessages.ERROR_MESSAGES_HOLDER, Collections.singletonList("basket.next.selection.criteria.not.met"));
            ...
            int pos = previousUrl.indexOf(previousRequest.getContextPath()) + previousRequest.getContextPath().length();
	...
	protected String internalEditComponent(final String bundleNo, final String componentId, final String searchQuery,
			...
			throws CMSItemNotFoundException
	{
		model.addAttribute("dashboard", guidedSellingFacade.getDashboard(Integer.valueOf(bundleNo).intValue(), componentId));
			...
			final String bundleTemplateId, final boolean removeCurrentProducts, final RedirectAttributes redirectModel)
	{
		...
		try
		{
			...
			for (final CartModificationData cartModification : cartModifications)
			{
                    ...
                    {
                        redirectModel.addFlashAttribute(GlobalMessages.ERROR_MESSAGES_HOLDER,
                                Collections.singletonList("basket.information.quantity.noItemsAdded." + cartModification.getStatusCode()));
                    }
		...
		{
			redirectModel
					.addFlashAttribute(GlobalMessages.ERROR_MESSAGES_HOLDER, Collections.singletonList("basket.error.occurred"));
