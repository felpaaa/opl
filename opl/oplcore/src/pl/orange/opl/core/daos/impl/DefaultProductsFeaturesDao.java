 ...
 *  
 */
package pl.orange.opl.core.daos.impl;
...
import pl.orange.opl.core.daos.ProductsFeaturesDao;
...
public class DefaultProductsFeaturesDao extends AbstractItemDao implements ProductsFeaturesDao
{
	...
	private static final String FIND_PRODUCTS_BY_VENDORCDOE_QUERY = "SELECT {a:" + ProductFeatureModel.PK + "} FROM {"
			...
			+ ProductFeatureModel.PRODUCT + "} " + "	JOIN " + ClassAttributeAssignmentModel._TYPECODE + " AS caa ON {caa:pk} = {pf:"
			+ ProductFeatureModel.CLASSIFICATIONATTRIBUTEASSIGNMENT + "}" + "  JOIN " + ClassificationAttributeModel._TYPECODE
			+ " AS ca ON {ca:pk} = {caa: " + ClassAttributeAssignmentModel.CLASSIFICATIONATTRIBUTE + "}" + "  JOIN "
			+ ClassificationClassModel._TYPECODE + " AS cc ON {cc:pk} = {caa:" + ClassAttributeAssignmentModel.CLASSIFICATIONCLASS
			+ "}" + "}" + " WHERE {cc:" + ClassificationClassModel.CODE + "} = ?classificationClassCode AND {ca:"
			+ ClassificationAttributeModel.CODE + "} = ?classificationAttributeCode AND {pf:stringvalue} LIKE ?brand and {a:"
			+ ProductModel.ITEMTYPE + "}=?itemType";
