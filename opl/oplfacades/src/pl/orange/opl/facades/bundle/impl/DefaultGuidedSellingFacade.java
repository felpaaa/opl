 ...
 *  
 */
package pl.orange.opl.facades.bundle.impl;
...
import pl.orange.opl.core.model.DeviceModel;
import pl.orange.opl.core.model.ServiceAddOnModel;
import pl.orange.opl.core.model.ServicePlanModel;
import pl.orange.opl.facades.bundle.GuidedSellingFacade;
import de.hybris.platform.configurablebundlefacades.data.BundleTemplateData;
import pl.orange.opl.facades.data.BundleTemplateServiceAddOnsPopulatorParameters;
import pl.orange.opl.facades.data.DashboardData;
import pl.orange.opl.facades.data.DashboardPopulatorParameters;
...
public class DefaultGuidedSellingFacade implements GuidedSellingFacade
{
	...
	@Override
	public String getRelativeComponentId(final String bundleNo, final String bundleTemplateId, final int relativeposition)
	{
		final BundleTemplateModel bundleModel = getBundleTemplateService().getBundleTemplateForCode(bundleTemplateId);
	...
	public boolean checkIsComponentSelectionCriteriaMet(final String bundleNo, final String bundleTemplateId)
	{
		...
		final List<CartEntryModel> entries = getBundleCommerceCartService().getCartEntriesForBundle(cartModel, bundleNoValue);
        ...
        if(entries != null && entries.size() > 0) {
            ...
            return getBundleCommerceCartService().checkIsComponentSelectionCriteriaMet(cartModel, bundleModel, bundleNoValue);
        }
	...
	{
		final CartModel cartModel = getCartService().getSessionCart();
		final BundleTemplateModel bundleModel = getBundleTemplateService().getBundleTemplateForCode(componentId, componentVersion);
		...
		for (final ProductData productData : searchPageData.getResults())
		{
			...
			final ChangeProductPriceBundleRuleModel priceRule = getBundleRuleService().getChangePriceBundleRule(cartModel,
					bundleModel, product, bundleNo.intValue());
			SubscriptionPricePlanModel pricePlan = null;

			if (product instanceof SubscriptionProductModel)
			{
				pricePlan = getCommercePriceService().getSubscriptionPricePlanForProduct((SubscriptionProductModel) product);
			}
		...
		{
			final ProductModel product = getProductService().getProductForCode(productData.getCode());
			final String disableMessage = getBundleCommerceCartService().checkAndGetReasonForDisabledProductInComponent(cartModel,
					product, bundleModel, bundleNo.intValue(), true);
			...
			final String searchQuery, final String urlPrefix, final String componentId, final Integer bundleNo)
	{
		...
		final CartModel cartModel = getCartService().getSessionCart();
		final List<CartEntryModel> entries = getBundleCommerceCartService().getCartEntriesForBundle(cartModel, bundleNo.intValue());
