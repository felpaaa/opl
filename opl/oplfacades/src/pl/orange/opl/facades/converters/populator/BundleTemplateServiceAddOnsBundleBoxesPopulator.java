 ...
 *  
 */
package pl.orange.opl.facades.converters.populator;
...
import pl.orange.opl.core.bundle.GuidedSellingService;
import pl.orange.opl.core.model.ServiceAddOnModel;
import pl.orange.opl.facades.data.BundleBoxData;
import pl.orange.opl.facades.data.BundleBoxEntryData;
import de.hybris.platform.configurablebundlefacades.data.BundleTemplateData;
import pl.orange.opl.facades.data.BundleTemplateServiceAddOnsPopulatorParameters;
		...
		implements Populator<SOURCE, TARGET>
{
	...
	public void populate(final SOURCE source, final TARGET target) throws ConversionException
	{
		...
		{
			if (getGuidedSellingService().isComponentToBeDisplayedOnGuidedSellingSelectPage(cart, bundleTemplateModel, bundleNo,
					ServiceAddOnModel.class))
			{
				...
				// create a target list which shall contain only selected references from the original entries
				final List<BundleBoxEntryData> selectedBoxEntryDatas = new ArrayList<BundleBoxEntryData>();

				// Loop through the BoxEntryData and set order entry data if product exist in the cart
				for (final BundleBoxEntryData boxEntryData : bundleBoxData.getBundleBoxEntries())
				{
					populateBundleBoxEntryForDisabledProducts(cart, source.getBundleNo().intValue(), bundleTemplateModel, boxEntryData);
					...
					if (cartEntry == null)
					{
						boxEntryData.getProduct().setThisBundleProductPrice(
								calculateProvisionedPriceForNonCartEntry(cart, source.getBundleNo().intValue(), bundleTemplateModel,
										boxEntryData.getProduct()));
	...
	{
		final ChangeProductPriceBundleRuleModel reducedPriceRule = getBundleRuleService().getChangePriceBundleRule(cart,
				bundleTemplateModel, getProductService().getProductForCode(productData.getCode()), bundleNo);
		...
		if (reducedPriceRule == null)
		{
			if (productData.getPrice() instanceof SubscriptionPricePlanData)
			{
				...
				{
					final RecurringChargeEntryData firstChargeEntry = pricePlan.getRecurringChargeEntries().iterator().next();
			...
			final BundleTemplateModel bundleTemplateModel, final BundleBoxEntryData boxEntryData)
	{
		...
		final String disableMessage = getBundleCommerceCartService().checkAndGetReasonForDisabledProductInComponent(cart,
				productModel, bundleTemplateModel, bundleNo, false);
	...
	protected BundleTemplateModel findExpandedComponent(final CartModel cart, final BundleTemplateModel sourceBundleTemplateModel,
			final List<BundleTemplateModel> childBundleTemplates, final int bundleNo)
	{
	...
	{
		final List<CartEntryModel> cartEntriesForProductInBundle = getBundleCommerceCartService().getCartEntriesForProductInBundle(
				cart, getProductService().getProductForCode(productCodeToFind), bundleNoToFind);
