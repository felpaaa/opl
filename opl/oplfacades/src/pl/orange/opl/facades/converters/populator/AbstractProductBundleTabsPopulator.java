...
import pl.orange.opl.facades.product.FrequencyTabDataComparator;
			...
			for (final ProductModel targetProductModel : getProducts(productModel, sourceComponent, targetComponent))
			{
				if (targetProductModel instanceof SubscriptionProductModel)
				{
					...
					final TermOfServiceFrequencyData termOfServiceFrequency = getTermOfServiceFrequencyConverter().convert(
							subscriptionProductModel.getSubscriptionTerm().getTermOfServiceFrequency());

					final int termOfServiceNumber = subscriptionProductModel.getSubscriptionTerm().getTermOfServiceNumber() == null ? 0
							: subscriptionProductModel.getSubscriptionTerm().getTermOfServiceNumber().intValue();
					...
					callPopulators(sourceComponent, targetComponent, productModel, productData, subscriptionProductModel,
				...
				{
					LOG.error("Product '" + targetProductModel.getCode() + "' is not a SubscriptionProduct. Ignoring it.");
				}
			}
