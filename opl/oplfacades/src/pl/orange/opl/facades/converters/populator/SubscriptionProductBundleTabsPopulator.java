...
/**
 * Populates the {@link ProductData} with the plans for guided selling bundle information to be used by front end.
 */
		...
		extends AbstractProductBundleTabsPopulator<SOURCEPRODUCT, TARGETPRODUCT, SOURCETEMPLATE>
{
	...
	/**
	 * Resolves the components to be used for populating the bundle tabs. For Plans we are using all existing components
	 * that have plans as products, no matter if they are assigned to this <code>productModel</code> or not. So for any
	 * plan we will find all existing plan components.
	 * 
	 * @param productModel
	...
	protected Collection<SOURCETEMPLATE> getComponents(final SOURCEPRODUCT productModel)
	{
		...
		final Collection<SOURCETEMPLATE> bundleTemplateModels = (List<SOURCETEMPLATE>) getBundleTemplateService()
				.getAllApprovedRootBundleTemplates(productModel.getCatalogVersion());
		...
		{
			for (final BundleTemplateModel component : getBundleTemplateService().getAllComponentsOfType(bundleTemplateModel,
					typeClass))
			{
	 ...
	 * case of a Plan, the target component the same as the source component as it is not used.
	 * 
	...
	/**
	 * Resolves the list of {@link ProductModel}s for the given <code>sourceComponent</code>. For plans it simply
	 * resolves all products for the source component {@link BundleTemplateModel}.
	 * 
	 * @param productModel
	...
	/**
	 * Hook to call additional populators for different purposes. For Plans we are populating the standard price of the
	 * according plan. Afterwards we are populating the classification data of the plan.
	 * 
	 * @param sourceComponent
